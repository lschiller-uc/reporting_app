...
contributors_agg AS ( --Step #5 Check for instances that match on the ISBN or the OCN in the instances attached to the PO.
    ...
    LEFT JOIN folio_orders.po_line__t plt ON plt.id = pol2.id
    GROUP BY plt.instance_id
    ),
pub_dates_agg AS (
    SELECT 
        plt2.instance_id AS instance_id,
        string_agg(plt2.publication_date, ' | ') AS dates
    FROM folio_orders.po_line__t plt2 
    GROUP BY plt2.instance_id
    ),
editions_agg AS (
    SELECT 
        plt3.instance_id AS instance_id,
        string_agg(plt3.edition, ' | ') AS editions
    FROM folio_orders.po_line__t plt3 
    GROUP BY plt3.instance_id
    ),
holdings_agg AS (
    SELECT
...
select DISTINCT ON (ord.po_instance_hrid, matched_ids.instance_hrid)
...
FROM new_orders ord 
JOIN identifiers AS matched_ids ON ord.order_identifier = matched_ids.identifier
LEFT JOIN folio_inventory.instance__t iton ON iton.id = matched_ids.instance_id
LEFT JOIN folio_inventory.INSTANCE i ON i.id = iton.id
    LEFT JOIN LATERAL jsonb_array_elements(jsonb_extract_path(i.jsonb, 'statisticalCodeIds')) AS stat_codes (jsonb) ON TRUE 
...
LEFT JOIN contributors_agg AS cont ON matched_ids.instance_id = cont.instance_id
LEFT JOIN pub_dates_agg AS pd ON matched_ids.instance_id = pd.instance_id
LEFT JOIN editions_agg AS ed ON matched_ids.instance_id = ed.instance_id
LEFT JOIN holdings_agg AS ha ON matched_ids.instance_id = ha.instance_id
WHERE ord.po_instance_hrid != matched_ids.instance_hrid
$$
LANGUAGE SQL
STABLE
PARALLEL SAFE;
...
SELECT * FROM DuplicateOrders('2026-09-03', '2026-09-04');
